...
export default function FacilitiesPage() {
  ...
  const sections = data.facilitiesPage || {};

  // Ambil gambar untuk carousel dari facilitiesPage, gallery, atau facilitiesMain
  const facilitiesCarouselImages = sections.facilitiesCarouselImages || [];
  
  // Jika tidak ada di facilitiesPage, ambil dari gallery atau facilitiesMain
  ...
  const images = facilitiesCarouselImages.length > 0 
    ? facilitiesCarouselImages 
    : (carouselImages.length > 0 ? carouselImages : fallbackImages);

  // Gambar untuk carousel aktivitas
  const activityCarouselImages = sections.activityCarouselImages || [];
  const fallbackActivityImages = activities
    .filter((a) => a.image)
    .map((a) => a.image!)
    .concat(
      gallery
    ...
    .slice(0, 8);
  const activityImages = activityCarouselImages.length > 0 ? activityCarouselImages : fallbackActivityImages;
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-4 py-16 md:py-24 text-left">
          <div className="max-w-2xl space-y-8 md:space-y-10">
            ...
            <p className="text-base md:text-lg leading-relaxed text-[#F3F4F6]">
              {sections.heroDescription || "Memberikan dukungan hunian penuh waktu dengan pendampingan menyeluruh setiap hari menghadirkan kenyamanan, keamanan, dan perhatian yang konsisten."}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-2xl overflow-hidden flex flex-col md:flex-row gap-0 shadow-lg">
            ...
            <div className="md:w-1/2 aspect-video md:aspect-auto md:min-h-[500px] relative bg-atedia-cream rounded-t-2xl md:rounded-t-none md:rounded-l-2xl overflow-hidden">
              {sections.seniorLivingImage || main[0]?.image ? (
                <Image
                  src={sections.seniorLivingImage || main[0].image!}
            ...
            <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white">
              ...
              <h4 className="text-2xl md:text-3xl font-bold text-black mb-6">
                {sections.seniorLivingSubtitle || "Kehangatan untuk Hari Tua yang Bahagia"}
              </h4>
              <p className="text-black mb-8 text-base md:text-lg leading-relaxed">
                {sections.seniorLivingDescription || "Kami menghadirkan program perawatan fleksibel, dari hunian penuh hingga kegiatan sosial yang mendukung kesejahteraan dan kebahagiaan warga."}
              </p>
              {/* Info Boxes */}
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-atedia-sage rounded-lg p-4 flex flex-col items-center text-center">
                  ...
                  <p className="text-black text-xs font-medium mb-1">{sections.roomTypeLabel || "Tipe Kamar"}</p>
                  <p className="text-black text-sm font-semibold">{sections.roomTypeValue || "Personal | Sharing"}</p>
                </div>
                {/* Box 2 - Layanan */}
                <div className="bg-atedia-sage rounded-lg p-4 flex flex-col items-center text-center">
                  <svg className="w-8 h-8 text-white mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-black text-xs font-medium mb-1">{sections.serviceLabel || "Layanan"}</p>
                  <p className="text-black text-sm font-semibold">{sections.serviceValue || "24 Jam | Fleksibel"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center">
            ...
            <div className="w-full md:w-1/2 space-y-6">
              <h2 className="text-3xl md:text-4xl font-bold text-atedia-sage">
                {sections.facilitiesTitle || "Fasilitas Kami"}
              </h2>
              {sections.facilitiesParagraph1 && (
                <p className="text-atedia-brown text-base md:text-lg leading-relaxed">
                  {sections.facilitiesParagraph1}
                </p>
              )}
              {sections.facilitiesParagraph2 && (
                <p className="text-atedia-brown text-base md:text-lg leading-relaxed">
                  {sections.facilitiesParagraph2}
                </p>
              )}
              {!sections.facilitiesParagraph1 && !sections.facilitiesParagraph2 && (
                <>
                  <p className="text-atedia-brown text-base md:text-lg leading-relaxed">
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="w-full">
            <ImageCarousel images={activityImages} autoSlideInterval={3000} />
          </div>
        </div>
      </section>
